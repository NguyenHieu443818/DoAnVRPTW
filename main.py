...
from ultility.utilities import round_float, write_excel_file, visualize_clusters, create_graph
import signal
import logging
import os
import time
...
if __name__ == "__main__":
    ...
    algorithms = ['GA', 'GA_VNS']  # 'GA','PSO','ACO', 'GA_VNS'
    data_names = ['R1','R2','C1','C2','RC1','RC2']
    # ===============================Thiết lập các thông số===============================
    # Thông số VRPTW
    VEHCICLE_CAPACITY = 0  # Trọng tải tối đa của 1 xe
    ...
    M = 2
    # Thông số chạy GA
    INDIVIDUAL = 100  # Số lượng cá thể
    GENERATION = 100  # Số lượng đời quần thể
    CROSSOVER_RATE = 0.8  # Tỉ lệ trao đổi chéo
    MUTATION_RATE = 0.15  # Tỉ lệ đột biến
    CONSERVE_RATE = 0.1  # Tỉ lệ bảo tồn
    # Thông số chạy PSO
    NUM_PARTICLES = 100  # số hạt trong đàn
    MAX_ITER_PSO = 100  # Số vòng lặp thực hiện
    ...
    SIGM = 3  # Hệ số trong quá trình update feromone 
    RO = 0.8  # Hệ số bay hơi
    TH = 80  # Hệ số điều chỉnh bổ sung trong update pheromone
    NUM_ANTS = 100  # Số lượng kiến
    ...
    for data_name in data_names:
        ...
        logging.basicConfig(filename='terminal_output.log',
        ...
        try:
            ...
            for idx_al, algorithm in enumerate(algorithms):
                ...
                for idx_dat, data_file in enumerate(data_files):
                    ...
                    # Khởi tạo dữ liệu
                    vehicle_capacity, data, customers = load_txt_dataset(
                    ...
                    # visualize_clusters(data, np.concatenate(cluster), V1)
                    # exit()
                        ...
                        match algorithm:
                            ...
                            case "GA_VNS":
                                al = GA_VNS(individual=INDIVIDUAL, generation=GENERATION, crossover_rate=CROSSOVER_RATE, mutation_rate=MUTATION_RATE,
                                            vehcicle_capacity=VEHCICLE_CAPACITY, conserve_rate=CONSERVE_RATE, M=M, customers=customers, graph_data=graph_data, list_n_l=LIST_N_L, beta_0=BETA_0,
                                            beta_1=BETA_1)
