...
Customer = namedtuple(
    'Customer', ['id', 'xy_coord', 'demand', 'readyTime', 'dueTime', 'serviceTime'])


def load_csv_dataset(url=None, name_of_id=None, number_of_customer: int = None):
    # folder = file_name[0:2]
    # url = "D:\\Python\\Genetic_Algorithm\\data\\csv\\" + folder +"\\"+ name_of_id+ ".csv"
    ...
    for i in range(number_of_customer+1):
        ...
        customers.append(
            Customer(i, data[i], demands[i], ready_time[i], due_date[i], service_time[i]))
...
def load_txt_dataset(url=None, name_of_id=None):
    ...
    for idx, line in enumerate(lines[9:]):  # Skip the header line
        ...
        customers.append(Customer(idx, np.array(
            [xcoord, ycoord]), demand, ready_time, due_date, service_time))

    # return data,customers,warehouse
    return vehicle_capacity, np.array(cord_data), customers
